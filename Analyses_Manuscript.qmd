...
## use the following work-around so brms does not complain 
renv::install("rstan")
...
Fig_01 <- ggplot(data = Fig_01.1_data, 
  ...
  geom_segment(x = 10, y = 1.115,
                 xend = 10, yend = 1.16, 
                 linetype = 2) +
  geom_segment(x = 17.5, y = 1.285,
                 xend = 22.5, yend = 1.285) +
  geom_segment(x = 22.5, y = 1.285,
               xend = 22.5, yend = 1.4, 
               linetype = 2) +
...
## create Fig. 01_bw
Fig_01_bw <- ggplot(data = Fig_01.1_data, 
       aes(x = x, 
           y = y)) + 
  geom_point(color = "lightgrey") + 
  geom_segment(x = 2.89, y = 1.295,
               xend = 7.5, yend = 1.295) +
  geom_segment(x = 7.5, y = 1.295,
               xend = 7.5, yend = 1.425, 
               linetype = 2) +
  geom_segment(x = 15.39, y = 1.51,
               xend = 20, yend = 1.51) +
  geom_segment(x = 20, y = 1.51,
               xend = 20, yend = 1.55, 
               linetype = 2) +
  ...
  annotate("text", label = "2", 
    ...
    geom_point(data = Fig_01.2_data, 
               aes(x = x, 
                   y = y), 
               color = "darkgrey") + 
    geom_segment(x = 5.39, y = 1.115,
                 xend = 10, yend = 1.115) +
    geom_segment(x = 10, y = 1.115,
                 xend = 10, yend = 1.16, 
                 linetype = 2) +
    geom_segment(x = 17.89, y = 1.285,
                 xend = 22.5, yend = 1.285) +
    geom_segment(x = 22.5, y = 1.285,
                 xend = 22.5, yend = 1.4, 
                 linetype = 2) +
...
## save Fig. 01_bw
ggsave(file = "Output_Graphics/Fig_01_bw.jpg", 
       Fig_01_bw, 
       width = 16, 
       height = 9, 
       dpi = 600,
...
color_uneven_rows <- color_even_rows <- "white"
column_names_color <- "#EEEEEE"
...
## save Fig. 04_bw
ggsave(file = "Output_Graphics/Fig_04_bw.jpg", 
       Fig_04_bw, 
       width = 16, 
       height = 9, 
       units = "cm", 
...
plot_df_row_2 <- data.frame(
  x = x, 
  y = 1 * x, 
  color = as.factor(rep(c(0.8, 0.2), each = 4))
)
...
## Select Control Conditions Designs and Reference Group Designs 
...
reference_group <- dplyr::setdiff(quantitative, control_condition)

## exclude reference group designs without baselines 
no_baselines <- c("ML2_Hsee", "ML1_Oppenheimer_Monin", "ML2_Huang_CR", "ML2_Huang")
reference_group <- reference_group[!reference_group %in% no_baselines]

## n control_condition
length(reference_group)
...
all_designs_ID <- c(
  sub("_", replacement = "__", control_condition)[1:4], 
  "RRR_04__Sripada", 
  sub("_", replacement = "__", reference_group)[1:length(reference_group)]
)
...
Table_1 <- data.frame(
  ...
  Design = c(rep("control condition", times = length(control_condition)),
             rep("reference group", times = length(reference_group))),
...
control_condition_short <- stringr::str_remove(control_condition,  "ML1_")
control_condition_short <- stringr::str_remove(control_condition_short,  "ML2_")
control_condition_short <- stringr::str_remove(control_condition_short,  "RRR_04_")


## run the model
control_condition_brms <- lapply(control_condition_short,
                           function(name){
...
## rename list objects
names(control_condition_brms) <- control_condition

## export results
readr::write_rds(control_condition_brms, "Output_Data/control_condition_brms.rds")

## import results: if the data exist, this saves a lot of time
control_condition_brms <- readr::read_rds("Output_Data/control_condition_brms.rds")

```

#### Reference Groups Designs

```{r}

## remove multi-lab ID for further use
reference_group_short <- stringr::str_remove(reference_group,  "ML1_")
reference_group_short <- stringr::str_remove(reference_group_short,  "ML2_")
reference_group_short <- stringr::str_remove(reference_group_short,  "ML3_")

## run the model
reference_group_brms <- lapply(reference_group_short,
                           function(name){
                              # random intercepts & random slopes
                              brms::brm(
                                brms::bf(
...
## rename list objects
names(reference_group_brms) <- reference_group

## export results
readr::write_rds(reference_group_brms, "Output_Data/reference_group_brms.rds")

## import results: if the data exist, this saves a lot of time
reference_group_brms <- readr::read_rds("Output_Data/reference_group_brms.rds")
...
names(primary_two_stage_MA_CM) <- control_condition
...
secondary_two_stage_MA_CM <- lapply(
  reference_group[-which(reference_group == "ML1_Jacowitz_Population")], 
  function(MASC_name){
         # select data set 
         MASC_data <- Site_Summaries$Site_Summaries[[MASC_name]]
         # run meta-analysis 
...
names(secondary_two_stage_MA_CM) <- reference_group[-which(reference_group == "ML1_Jacowitz_Population")]
...
names(secondary_two_stage_MA_TM) <- reference_group[-which(reference_group == "ML1_Jacowitz_Population")]
...
helper_fun <- function(brms_object_name, background_color){
  ...
  brms_object = control_condition_brms[[brms_object_name]],
...
row_1__col_B <- helper_fun(control_condition[1], 
                           background_color = color_uneven_rows)
row_2__col_B <- helper_fun(control_condition[2], 
                           background_color = color_even_rows)
row_3__col_B <- helper_fun(control_condition[3], 
                           background_color = color_uneven_rows)
row_4__col_B <- helper_fun(control_condition[4], 
                           background_color = color_even_rows)
row_5__col_B <- helper_fun(control_condition[5], 
                           background_color = color_uneven_rows)
...
row_1__col_C <- helper_fun(control_condition[1], 
                           background_color = color_uneven_rows)
row_2__col_C <- helper_fun(control_condition[2], 
                           background_color = color_even_rows)
row_3__col_C <- helper_fun(control_condition[3], 
                           background_color = color_uneven_rows)
row_4__col_C <- helper_fun(control_condition[4], 
                           background_color = color_even_rows)
row_5__col_C <- helper_fun(control_condition[5], 
                           background_color = color_uneven_rows)
...
## create Fig. 07
Fig_07_bw <- gridExtra::grid.arrange(
  grobs = list(
    plot_grid_name_col_0, 
    plot_grid_name_col_A, 
    plot_grid_name_col_B, 
...
## save Fig. 07_bw
ggsave(file = "Output_Graphics/Fig_07_bw.png", 
       Fig_07_bw, 
       width = 16, 
       height = 4 * 5.25, 
       units = "cm", 
...
row_1__col_A <- descriptors_reference_group_design(
  name = reference_group[1], 
  brms_data = reference_group_brms, 
...
helper_fun <- function(brms_object_name, background_color){
  MD_heterogeneity_plot_2(
  brms_object = reference_group_brms[[brms_object_name]],
  return_plot_list = FALSE, 
  DV_lim_lower = NULL, 
  DV_lim_upper = NULL, 
  label_short = TRUE, 
...
## save Fig. 08.1
ggsave(file = "Output_Graphics/Fig_08.1.jpg", 
       Fig_08.1, 
       width = 16, 
       height = 4 * 6.25, 
       units = "cm", 
...
## save Fig. 08.2
ggsave(file = "Output_Graphics/Fig_08.2.jpg", 
       Fig_08.2, 
       width = 16, 
       height = 4 * 2.25, 
       units = "cm", 
...
## create Fig. 08.2_bw
Fig_08.2_bw <- gridExtra::grid.arrange(
  grobs = list(
    plot_grid_name_col_0, 
    plot_grid_name_col_A, 
    plot_grid_name_col_B, 
...
## save Fig. 08.2_bw
ggsave(file = "Output_Graphics/Fig_08.2_bw.jpg", 
       Fig_08.2_bw, 
       width = 16, 
       height = 4 * 2.25, 
       units = "cm", 
