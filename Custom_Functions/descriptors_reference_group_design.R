...
descriptors_reference_group_design <- function(name, brms_data, background_color, two_stage_MA_CM, two_stage_MA_TM, two_stage_MA_MD){
  ...
  ggplot() + 
    ...
    annotate(
      "text", label = CM_het,
      x = 3, y = 3.75, size = 4, colour = "black", parse = TRUE
    ) +
    annotate(
      "text", label = TM_het,
      x = 3, y = 3, size = 4, colour = "black", parse = TRUE
    ) +
    annotate(
      "text", label = ISC,
      x = 3, y = 2.25, size = 4, colour = "black", 
